...
public class CommonService {
	...
	public void restViewProc(String restNo, int currentPage, Model model, HttpServletRequest req) {
		...
		int pageBlock = 3; //?òÏù¥ÏßÄ?? ?úÏãú ??
		int end = currentPage * pageBlock; //?òÏù¥ÏßÄ?? ?ùÎ≤à??
		int begin = end+1 - pageBlock; //?òÏù¥ÏßÄ?? ?úÏûë Î≤àÌò∏
		...
		int cntCollection = cDao.collCount(restNum); //Í¥Ä?? ?ùÎãπ?ºÎ°ú ?Ä?•Îêú Ï¥? ??
		int collection = cDao.collChck(cDto); // 1 -> ?Ä?•Îêú ?ùÎãπ, 0 -> ?Ä?•x
	...
	public int colletProc(String restNo) {
		...
		int result;
		if(check == 0) { //Ï§ëÎ≥µ ?ÑÎãå ?ÅÌÉúÎ°? ?Ä?? ÏßÑÌñâ
			result = cDao.collectProc(cDto); //?Ä?? ?±Í≥µ ?? 1 Î∞òÌôò
		}
		else result = 0; // ?Ä?•Îêú ?ÅÌÉúÎ©? 0 Î∞òÌôò
		return result;
	}
